...
word2vec_path = '../data/featurization/word2vec/'
...
datafolder = '../data/'
word2vec_data = lambda dim: os.path.join(current_path, Path(
                                         '../../../data/featurization/word2vec/word2vec_dsl_sentences_reddit_sentences_300_cbow_negative.kv'.format(
                                             dim)))
...
def load_saved_word_embeddings():
    global wv_model
    wv_model = KeyedVectors.load(word2vec_data(300))
    global vector_size
    vector_size = 300
    return wv_model
...
def full_comment_emb(tokens):
    ...
    if n == 0:
        return None
    return [wv_model[tokens[i]].tolist() for i in range(n) if tokens[i] in wv_model.vocab]
...
def main(argv):
    ...
    parser.add_argument('--train_algorithm', type=str, default='negative',
                        help='the training algorithm: "softmax" or "negative"')
    parser.add_argument('--workers', type=int, default=4, help='number of workers')
    parser.add_argument('--word2vec_format', action='store_true', default=False,
                        help='Store in the original C word2vec (.txt) format')
